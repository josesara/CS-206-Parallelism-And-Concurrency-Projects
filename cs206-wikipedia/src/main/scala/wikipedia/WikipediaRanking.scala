...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.parallelize(WikipediaData.lines).map(s => WikipediaData.parse(s))

  /** Returns the number of articles on which the language `lang` occurs.
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = 
      rdd.aggregate(0)((counter, article) => {
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = 
      rdd.flatMap(article => langs.filter(lang => article.text.split(" ").contains(lang)).map(lang => (lang, 1))).reduceByKey(_ + _).collect.toList.sortBy(_._2)(Ordering[Int].reverse)
